"""
Accounting tools implementation for MCP-Odoo
"""
...
def register_accounting_tools(mcp: FastMCP) -> None:
    """Registers accounting-related tools"""
    
    @mcp.tool(description="Search for journal entries with filters")
    def search_journal_entries(
        ctx: Context,
        filters: JournalEntryFilter
    ) -> Dict[str, Any]:
        """
        Search for journal entries based on specified filters
        
        Args:
            filters: Filters for journal entry search
        ...
        try:
            ...
            # For each entry, get summarized line information
            for entry in entries:
                if entry.get("line_ids"):
                    ...
                    entry["lines"] = lines
                    # Remove the ID list to reduce size
    ...
    @mcp.tool(description="Create a new journal entry")
    def create_journal_entry(
        ...
        Args:
            entry: Journal entry data to create
        ...
        try:
            # Verify that debit and credit balance
            ...
            # Get information about the created entry
